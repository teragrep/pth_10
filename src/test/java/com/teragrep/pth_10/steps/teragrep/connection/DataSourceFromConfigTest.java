...
public final class DataSourceFromConfigTest {
    ...
    public void missingUsernameThrows() {
        final Properties props = defaultProperties();
        props.remove("dpl.pth_10.archive.db.username");
        final Config config = ConfigFactory.parseProperties(props);
        final RuntimeException exception = Assertions
                .assertThrows(RuntimeException.class, new DataSourceFromConfig(config)::get);
        final String expected = "Missing configuration item: 'dpl.pth_10.archive.db.username'.";
        Assertions.assertEquals(expected, exception.getMessage());
    }
    ...
    public void missingPasswordThrows() {
        final Properties props = defaultProperties();
        props.remove("dpl.pth_10.archive.db.password");
        final Config config = ConfigFactory.parseProperties(props);
        final RuntimeException exception = Assertions
                .assertThrows(RuntimeException.class, new DataSourceFromConfig(config)::get);
        final String expected = "Missing configuration item: 'dpl.pth_10.archive.db.password'.";
        Assertions.assertEquals(expected, exception.getMessage());
    ...
    private Properties defaultProperties() {
        ...
        properties.put("dpl.pth_10.archive.db.username", username);
        properties.put("dpl.pth_10.archive.db.password", password);
        properties.put("dpl.pth_10.archive.db.url", url);
        return properties;
    }
}
