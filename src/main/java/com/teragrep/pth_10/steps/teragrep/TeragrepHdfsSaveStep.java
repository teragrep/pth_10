...
import com.teragrep.pth_10.ast.commands.transformstatement.teragrep.HdfsSaveMetadata;
...
public final class TeragrepHdfsSaveStep extends TeragrepHdfsStep {
    ...
    public Dataset<Row> get(Dataset<Row> dataset) throws StreamingQueryException {
        ...
        try {
            ...
            if (fs.exists(fsPath) && fs.isDirectory(fsPath)) {
                if (overwrite) {
                    LOGGER
                            .info(
                                    "queryId <{}> TG HDFS Save: Pre-existing data was found in specified path. Deleting pre-existing data",
                                    catCtx.getQueryName()
                            );
                    ...
                    if (
                        ...
                        LOGGER
                                .info(
                                        "queryId <{}> Previous HDFS save to this path was not streaming and appId matches last save; allowing overwrite and bypassing overwrite=false parameter",
                                        catCtx.getQueryName()
                                );
                    }
                    else {
        ...
        LOGGER.info("queryId <{}> Dataset in HDFS save was streaming={}", catCtx.getQueryName(), dataset.isStreaming());
