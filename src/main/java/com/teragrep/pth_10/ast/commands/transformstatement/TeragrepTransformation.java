 ...
 */
package com.teragrep.pth_10.ast.commands.transformstatement;
...
import com.teragrep.pth_10.ast.bo.StringNode;
import com.teragrep.pth_10.ast.bo.Token;
...
import com.teragrep.pth_10.ast.commands.transformstatement.teragrep.ContextBloomMode;
import com.teragrep.pth_10.ast.commands.transformstatement.teragrep.EstimateColumnFromBloomContext;
import com.teragrep.pth_10.ast.commands.transformstatement.teragrep.InputColumnFromBloomContext;
import com.teragrep.pth_10.ast.commands.transformstatement.teragrep.OutputColumnFromBloomContext;
...
public class TeragrepTransformation extends DPLParserBaseVisitor<Node> {
    ...
    private Node teragrepTransformationEmitCatalyst(DPLParser.TeragrepTransformationContext ctx) {
        ...
        if (zplnConfig != null) {
            ...
            if (zplnConfig.hasPath(hostCfgItem) && !zplnConfig.getString(hostCfgItem).equals("")) {
                LOGGER.info("queryId <{}> Host set via zeppelin config item", catCtx.getQueryName());
                this.host = zplnConfig.getString(hostCfgItem);
            }

            if (zplnConfig.hasPath(portCfgItem)) {
                LOGGER.info("queryId <{}> Port set via zeppelin config item", catCtx.getQueryName());
            ...
            if (zplnConfig.hasPath(enforceDestinationCfgItem)) {
                LOGGER.info("queryId <{}> Enforce destination set via zeppelin config item", catCtx.getQueryName());
    ...
    public Node visitT_hostParameter(DPLParser.T_hostParameterContext ctx) {
        ...
        LOGGER
                .info(
                        "queryId <{}> Teragrep command: Host and port set to <[{}]>:<[{}]>", catCtx.getQueryName(),
                        host, port
                );
        return null;
    }
    ...
    public Node visitT_listModeParameter(DPLParser.T_listModeParameterContext ctx) {
        ...
        if (ctx.t_pathParameter() != null) {
            ...
            LOGGER
                    .info(
                            "queryId <{}> Defaulting to home directory: Path was not provided to the hdfs list operation.",
                            catCtx.getQueryName()
                    );
        }
    ...
    public Node visitT_regexextractParameter(final DPLParser.T_regexextractParameterContext ctx) {
        ...
        if (ctx.t_regexParameter() != null) {
            ...
            LOGGER.info("queryId <{}> regexextract regex: <[{}]>", catCtx.getQueryName(), regex);
        }
        else {
