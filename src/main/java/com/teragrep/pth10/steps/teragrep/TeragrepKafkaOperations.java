...
public class TeragrepKafkaOperations {

    /**
     * Saves the input dataset to kafka with the specified topic
     * @param kafkaTopic kafka topic
     * @return input data passthrough
     */
    static Dataset<Row> saveDataToKafka(Dataset<Row> dataset, String hdfsPath,
                                               DPLParserCatalystContext catCtx, Config zeppelinConfig, String kafkaTopic) {
        ...
        if (!toKafkaDs.isStreaming()) {
            // save to ObjectStore, and send on flush
            // complete output mode, e.g. after aggregations and in forEachBatch
            DataFrameWriter<Row> kafkaWriter = toKafkaDs
                    .write()
                    .format("kafka")
            ...
            final String objId = "kafka-save"; // only save latest dataframe
            catCtx.getObjectStore().add(objId, kafkaWriter);
        }
        else {
