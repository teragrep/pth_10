...
/**
 * A class that processes a list of rows into an email.
 * Used for the sendemail command.
 */
public class SendemailResultsProcessor implements Serializable {
	...
	/**
	 * Used for testing translation; not to be used outside of testing and/or debugging.
	...
	try {
		...
		if (toEmails != null) {
			...
			throw new IllegalArgumentException("Sendemail command did not contain a required parameter: 'to=<email_list>' !");
		}
		...
		if (this.urlToParagraph != null) {
			if (this.content_type.equals("html")) {
				messageContent += String.format("<br><a href=\"%s\">View results in Teragrep</a>", this.urlToParagraph);
			}
			else {
				// plain
		...
		footerBodyPart.setContent(footerContent, content_type.equals("html") ? "text/html; charset=utf-8" : "text/plain; charset=utf-8");
		...
		messageBodyPart.setContent(messageContent, content_type.equals("html") ? "text/html; charset=utf-8" : "text/plain; charset=utf-8");
		...
		if (sendResults && inline) {
			...
			dataBodyPart.setContent(dataContent, content_type.equals("html") ? "text/html; charset=utf-8" : "text/plain; charset=utf-8");
		}
		...
		if (sendResults && sendCsv) {
			...
			try {
				...
				BaseTable table = new BaseTable(yPosition, yStartNewPage, bottomMargin, tableWidth, margin, pdfDoc, page1, true, drawContent);
