...
public class SendemailTransformation extends DPLParserBaseVisitor<Node> {
	...
	private Node sendemailTransformationEmitCatalyst(DPLParser.SendemailTransformationContext ctx) {
		...
		if (zplnConfig != null && zplnConfig.hasPath(smtpEncryptionParameterCfgItem)) {
			...
			if (value.equals("PLAIN")) {
				...
				throw new IllegalArgumentException("Invalid value for '" + smtpEncryptionParameterCfgItem + "'. It must be 'PLAIN', 'SSL' or 'TLS' instead of '" + value + "'");
			}
		}
		...
		for (int i = 0; i < ctx.getChildCount(); i++) {
			...
			if (child instanceof TerminalNode) {
				...
				throw new UnsupportedOperationException("Sendemail does not support 'width_sort_columns' parameter yet.");
		...
		if (restrictedMode && hasForbiddenConfig) {
			throw new IllegalArgumentException("Forbidden configuration detected. Please make sure that only the 'to' and 'subject' parameters are used, or switch off restricted mode.");
		}

		if (use_tls && use_ssl) {
			throw new IllegalArgumentException("Both 'use_tls' and 'use_ssl' cannot be used simultaneously. Please enable either 'use_tls' or 'use_ssl', not both.");
		}
	...
	@Override
	public Node visitT_sendemail_sendresultsOptionParameter(DPLParser.T_sendemail_sendresultsOptionParameterContext ctx) {
		Node rv = null;
	...
	@Override
	public Node visitT_sendemail_paperorientationOptionParameter(DPLParser.T_sendemail_paperorientationOptionParameterContext ctx) {
		Node rv = null;
	...
	@Override
	public Node visitT_sendemail_contentTypeOptionParameter(DPLParser.T_sendemail_contentTypeOptionParameterContext ctx) {
		Node rv = null;
	...
	@Override
	public Node visitT_sendemail_widthSortColumnsOptionParameter(DPLParser.T_sendemail_widthSortColumnsOptionParameterContext ctx) {
		Node rv = null;
