 ...
 * end is excluded from the resulting field,
 * step can be an integer or a timespan (string)<br>
 * 
 * While using a timespan, if the increment causes the
...
public class Mvrange implements UDF3<Integer, Integer, Object, List<String>>, Serializable {
	...
	public List<String> call(Integer start, Integer end, Object stepObj) throws Exception {
		...
		if (stepObj instanceof Long) {
			...
			throw new RuntimeException("Mvrange: Step increment argument could not be interpreted into a valid argument. Make sure the argument is an integer or a timespan.");
		}
