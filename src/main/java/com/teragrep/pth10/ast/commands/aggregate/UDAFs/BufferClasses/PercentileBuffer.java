...
/**
 * Buffer used for {@link com.teragrep.pth10.ast.commands.aggregate.UDAFs.ExactPercentileAggregator ExactPercentileAggregator}
 *
 */
public class PercentileBuffer extends ListBuffer<Double> implements Serializable {
	 ...
	 * otherwise get the middle value.
	 * @return percentile
	 */
	public double calculatePercentile() {
		...
		int index = -1;
		if (Math.abs(nonRoundedIndex - Math.ceil(nonRoundedIndex)) < Math.abs(nonRoundedIndex - Math.floor(nonRoundedIndex))) {
